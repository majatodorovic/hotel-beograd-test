...
export default function Gallery({ data }: any) {
  const initialNumberOfImages = useRef(12);
  const [currentNumberOfImages, setCurrentNumberOfImages] = useState(
    initialNumberOfImages.current
  );
  ...
  return (
    <section className="w-full flex flex-col items-center px-6 md:px-8 2xl:px-12">
      ...
      {currentNumberOfImages < data.items.length && (
        <button
          data-aos="fade-up"
          data-aos-easing="fadeUpCustom"
          onClick={() => {
            setCurrentNumberOfImages(
              currentNumberOfImages + initialNumberOfImages.current
            );
          }}
          className="text-xl md:text-2xl underline-after mt-16 md:mt-24 2xl:mt-36"
        >
          {data.loadMoreBtn}
        </button>
      )}
            ...
            >
              ...
              <div className="w-full h-full flex justify-center items-center shrink">
                ...
                <Swiper
                  ...
                  className="w-full flex items-center h-full shrink max-w-4xl mx-0"
                >
                  {data.items.map((img: any, index: any) => (
                    <SwiperSlide
                      key={index}
                      virtualIndex={index}
